"""Session evidence to sealed semantic proposals and exact document CAS plans."""
...
from memoryos.memory.documents.model import DocumentEditPlan, MemoryEditProposal, PresentPath
from memoryos.memory.documents.planner import MemoryDocumentPlanner
from memoryos.memory.documents.review import MemoryEditReviewStore
from memoryos.memory.evidence import DurableSalienceLedger, EpisodeSalienceGate, SessionArchiveEpisodeAdapter
from memoryos.memory.evidence.proposal_store import SealedProposalSet, SealedProposalStore
from memoryos.memory.extraction import MemoryExtractorBackend, RuleFallbackExtractor
from memoryos.memory.extraction.errors import MemoryExtractionError, classify_memory_extraction_failure
...
class MemoryCommitPlanner:
    """Call the model at most once, then route and plan deterministically."""
        ...
        *,
        ...
        for index, proposal in enumerate(sealed.proposals):
            ...
            if float(proposal.confidence) < self.auto_review_confidence_threshold:
                ...
                # Exact no-op plans do not need human authority because they
                # cannot mutate the source.  They remain on the idempotent
                # direct path so the Session consumer can close normally.
                if proposed_diff:
    ...
    def _sealed_proposals(
        ...
        if salience.salient:
            try:
                batch_method = getattr(self.extractor, "extract_batch_with_context", None)
    ...
    def _plan_change_digest(plan: DocumentEditPlan) -> str:
        """Fingerprint a planned effect without persisting Markdown bytes."""
