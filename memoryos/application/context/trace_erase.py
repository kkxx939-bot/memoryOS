"""Hard-erasure cleanup for tenant-scoped recall traces.

Recall traces deliberately have no durable document binding.  The cleanup
therefore removes every trace owned by the erased document's owner (plus
unscoped legacy traces) instead of trying to infer a content relationship.
"""
...
class RecallTraceEraseIntegrityError(RuntimeError):
    """A recall-trace directory cannot be safely and completely purged."""


class RecallTraceEraseBackend:
    """Remove owner-bound rebuildable recall traces during hard erase."""
                ...
                try:
                    ...
                    if isinstance(scope, dict):
                        ...
                        if trace_tenant and trace_tenant != tenant:
                            raise RecallTraceEraseIntegrityError(
                                "recall trace crosses its tenant directory"
                            )
                        trace_owner = str(scope.get("user_id") or scope.get("owner_user_id") or "")
                    if trace_owner:
                        try:
                            trace_owner = MemoryDocumentPathPolicy.trusted_segment(
                                trace_owner, "trace owner_user_id"
                            )
                        except ValueError as exc:
                            raise RecallTraceEraseIntegrityError(
                                "recall trace owner scope is invalid"
                            ) from exc
                    ...
                    if (
                        ...
                        raise RecallTraceEraseIntegrityError(
                            "recall trace changed while it was being erased"
                        )
    ...
    def _read_trace(directory_descriptor: int, name: str, trace_id: str) -> tuple[dict[str, Any], int]:
        ...
        try:
            ...
            if (
                ...
                or metadata.st_size > _MAX_TRACE_BYTES
            ):
