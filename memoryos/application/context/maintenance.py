"""Offline repair and verification for rebuildable serving projections."""
...
class DocumentServingMaintenance(Protocol):
    """Bounded Markdown-source rebuild boundary injected by the runtime."""
...
class CallbackDocumentServingMaintenance:
    """Compose a strict full-scan, publication and verification sequence.

    The adapter never discovers users by recursively walking the runtime root.
    A tenant-wide run requires a bounded owner provider.  Verification receives
    the exact safe scan generation that preceded publication, allowing the
    injected verifier to compare every live document ID, path and source digest
    with the disposable Catalog generation.
    """
    ...
    def _safe_scan(self, tenant_id: str, owner_user_id: str) -> Any:
        ...
        if (
            str(getattr(scan, "tenant_id", "")) != tenant_id
            or str(getattr(scan, "owner_user_id", "")) != owner_user_id
        ):
...
class CatalogDocumentProjectionVerifier:
    """Compare one safe Markdown scan with the tenant Catalog generation."""
...
class DerivedServingMaintenanceService:
    """Repair ordinary rows, then rebuild Markdown document projections."""
    ...
    def rebuild_index(self, *, owner_user_id: str | None = None) -> dict[str, Any]:
        """Run an offline bounded rebuild without treating Catalog as source."""

        self._require_ready()
        owner = (
            _trusted_scope_segment(owner_user_id, "owner_user_id")
            if owner_user_id is not None
            else None
        )
        try:
            with self.serving_lock:
                ...
                payload["consistent"] = bool(
                    ordinary.consistent and documents.get("consistent") is True
                )
                return payload
