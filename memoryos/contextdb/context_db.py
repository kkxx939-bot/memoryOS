...
from memoryos.contextdb.ordinary_relations import ordinary_relation_serving_eligibility
...
class ContextDB:
    ...
    def _add_relation_unfenced(self, relation: ContextRelation) -> None:
        self._require_ready()
        canonical_source = self._canonical_uri(relation.source_uri)
        ...
        for uri in (relation.source_uri, relation.target_uri):
            ...
            if uri == relation.source_uri and self._canonical_object(endpoint):
                canonical_source = True
        if canonical_source:
            raise PermissionError(
                "canonical relations with a canonical Source can only be published from an immutable receipt"
            )
        ...
        if self._canonical_object(authority):
            raise PermissionError("ordinary relation requires a non-canonical Source authority")
        ...
        for uri, endpoint in endpoint_objects.items():
            ...
            if (
                not self._canonical_object(endpoint)
                and endpoint_owner
        ...
        committer.commit(owner_user_id, [operation])

    def _ordinary_relation_committer(self, tenant_id: str) -> OperationCommitter:
        committer = self.committer
        if committer is None:
            root = getattr(self.source_store, "root", None)
            if root is None:
                raise RuntimeError("ContextDB.add_relation requires OperationCommitter for this SourceStore")
            committer = OperationCommitter(
                ...
                tenant_id=tenant_id,
                tombstone_service=self.tombstone_service,
                migration_gate=self.migration_gate,
            )
            self.committer = committer
        if (
    ...
    def commit_session(self, archive: SessionArchive, *, async_commit: bool = True) -> SessionCommitResult:
        ...
        if async_commit:
            try:
                ...
                archive_store = self.session_commit_service.archive_store
                if archive_store.archive_exists(
                    archive.archive_uri,
                    tenant_id=archive_store.tenant_id,
                ):
                    try:
                        self.session_commit_service.enqueue_failed_inline_commit(archive)
    ...
    def _rebuild_owner_index_locked(self, *, owner_user_id: str) -> dict:
        ...
        try:
            ...
            if self.current_slot_projector is not None and (
                ...
                from memoryos.contextdb.unified_migration import CurrentSlotMigrationBackfill
    ...
    def _rebuild_all_serving_locked(
        ...
        *,
        ...
        from memoryos.contextdb.store.vector_store import vector_capabilities, vector_row_id
