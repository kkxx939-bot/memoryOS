"""Serializable contracts for legacy and unified context retrieval queries."""
...
from memoryos.contextdb.catalog import normalize_timestamp as normalize_catalog_timestamp
from memoryos.contextdb.catalog import normalize_tree_path
...
class RetrievalQueryIntent(str, Enum):
    """Caller-visible retrieval semantics."""
...
class RetrievalOptions:
    """Structured retrieval options shared by all transports.

    Time ranges use half-open semantics: ``from <= value < to``. A date-only
    lower bound is the start of that date in ``timezone``; a date-only upper
    bound is the start of the following local date. All indexed timestamps are
    serialized as UTC ISO-8601 values.
    """
    ...
    def __post_init__(self) -> None:
        ...
        token_budget = _bounded_integer(self.token_budget, label="token_budget", maximum=MAX_TOKEN_BUDGET)
...
class RetrievalQueryPlan(RetrievalOptions):
    """A fully normalized and trusted-scope-bound online retrieval plan."""
...
@dataclass(frozen=True)
class QueryPlan:
    """Compatibility contract for the original hierarchical retriever.

    New online retrieval code should use :class:`RetrievalQueryPlan`. Keeping
    this class unchanged preserves existing imports and positional callers.
    """
...
def _normalize_timestamp(
    ...
    *,
    ...
    # SQLite compares the Catalog's indexed TEXT timestamps lexicographically.
    # Reuse the Catalog writer's canonical UTC representation so an exact
    # boundary is byte-for-byte equal to the stored value.  Mixing ``Z`` here
    # with the Catalog's ``+00:00`` representation makes an inclusive lower
    # bound skip the exact instant and an exclusive upper bound include it.
