"""Bounded exact validation for canonical retrieval candidates."""
...
class BoundedCanonicalResolver:
        ...
        for candidate in candidates:
            if not candidate.canonical_slot_id and not candidate.canonical_claim_id:
                ...
                if context_type == "memory" and (
                    not plan.owner_user_id
                    or str(candidate.metadata.get("owner_user_id") or "") != plan.owner_user_id
                ):
                    dropped.append(
                        {
    ...
    def _safe_candidate_metadata(metadata: Any) -> dict[str, Any]:
        ...
        safe = {
            field: raw[field]
            for field in _SAFE_CANDIDATE_METADATA_FIELDS
            if field in raw
        }
