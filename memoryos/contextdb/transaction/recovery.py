...
class RecoveryService:
        ...
        for path in sorted(outbox_root.glob("*.json")):
            ...
            try:
                ...
                transaction_entries = [
                    ...
                    if str(entry.operation.payload.get("transaction_id") or "")
                    == str(envelope["transaction_id"])
                ]
                recovered.extend(
                    self.committer.resume_canonical_batch(str(envelope["user_id"]), transaction_entries)
                )
    ...
    def _seed_redo_from_outbox(
        ...
        effects = {
            ...
            for item in envelope.get("effect_manifests", [])
            if isinstance(item, dict)
        }
        before = {
            str(item["uri"]): item
            for item in envelope.get("before_images", [])
            if isinstance(item, dict)
        }
        for operation in operations:
            ...
            try:
                ...
                if envelope["status"] == "source_committed":
                    raise RedoIntegrityError(
                        "source_committed outbox is missing its Source effect"
                    ) from None
                self.redo_log.begin(
                    operation,
                    phase="started",
