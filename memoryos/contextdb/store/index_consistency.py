"""Tenant-scoped consistency checks for ordinary Context serving rows."""
...
def prepare_generic_index_rebuild(
    ...
    *,
    ...
    """Remove only ordinary rows that can be recreated from ``SourceStore``.

    Session and domain projections use a Catalog key distinct from their
    public URI.  They remain in place until their own projector publishes a
    replacement.  An owner-scoped repair never mutates another owner's rows.
    """
...
class IndexConsistencyService:
    """Verify or rebuild ordinary Context indexes for exactly one tenant."""
