...
from memoryos.contextdb.store.source_store import is_canonical_memory_uri
...
class SQLiteRelationStore:
    ...
    def clear_ordinary_relations(self, *, tenant_id: str, limit: int) -> int:
        """Delete one bounded tenant batch while preserving canonical edges."""
        ...
        with self._connect() as conn:
            rows = conn.execute(
                ...
                FROM relations
                WHERE tenant_id = ?
                  AND instr(source_uri, '/memories/canonical/') = 0
                  AND instr(source_uri, '/memories/pending/') = 0
                ORDER BY source_uri, relation_type, target_uri
                LIMIT ?
                """,
                (tenant_id, maximum),
