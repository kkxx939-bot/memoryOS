"""Generic scope identity, applicability, visibility, and authority boundaries."""
...
@dataclass(frozen=True)
class ContextScope:
    """Generic scoped-context envelope shared by domain extensions."""
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, Any]) -> ContextScope:
        _mapping(payload, "context scope")
        for field_name in ("applicability", "visibility", "authority"):
            if field_name not in payload or not isinstance(payload[field_name], Mapping):
                raise ValueError(f"context scope {field_name} must be an object")
        raw_origin = payload.get("origin_refs", [])
        if not isinstance(raw_origin, Sequence) or isinstance(raw_origin, str | bytes):
            raise ValueError("context scope origin_refs must be an array")
        if any(not isinstance(item, Mapping) for item in raw_origin):
            raise ValueError("context scope origin_refs must contain scope objects")
        subject = payload.get("canonical_subject")
        if subject is not None and not isinstance(subject, Mapping):
            raise ValueError("context scope canonical_subject must be an object or null")
        return cls(
