...
def relation_effects_from_manifest(manifest: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    expected_identities = {
        canonical_json(relation_identity(effect["identity"])) for effect in effects
    }
...
def validate_marker(
    ...
    *,
    ...
    selected_object_effects = [
        ...
        or (isinstance(effect, dict) and str(effect.get("uri") or "") in object_uris)
    ]
    if object_uris is not None and {
        str(effect.get("uri") or "")
...
def _validate_object_effect(
    ...
    *,
    ...
    try:
        ...
        if effect.get("object_digest") != canonical_digest(None) or effect.get("content_digest") != canonical_digest(None):
