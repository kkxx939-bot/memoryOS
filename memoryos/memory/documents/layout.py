"""Fail-fast greenfield runtime layout for Markdown memory."""
    ...
    def _contains_legacy_state(self) -> bool:
        ...
        for child in tenant_root.iterdir():
            if child in allowed_empty_parents and child.is_dir() and not any(child.iterdir()):
                continue
            # A non-default tenant root sits below a shared ``tenants`` tree;
            # only that exact tenant namespace is considered here.
            return True
        return False
