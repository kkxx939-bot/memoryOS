"""Sealed, CAS-bound review proposals for Markdown memory edits."""
...
from memoryos.core.durable_io import (
...
class MemoryEditReviewIntegrityError(RuntimeError):
    """A sealed review record or its exact body blob failed validation."""


@dataclass(frozen=True)
class ReviewConsolidationSource:
    """Content-free exact source binding sealed into a consolidation review."""
...
class MemoryEditReviewStore:
    """Durable sealed review metadata with enumerable body-bearing blobs."""
    ...
    def seal(
        ...
        *,
        ...
        references = tuple(
            sorted(
                {
                    _independent_evidence_reference(item, owner_user_id=owner)
                    for item in independent_evidence_references
                }
            )
        )
        if len(references) > _MAX_INDEPENDENT_EVIDENCE_REFERENCES:
        ...
        with self.erasure_store.document_lock(tenant, owner, identifier):
            with self._owner_review_lock(tenant, owner):
                ...
                try:
                    ...
                    pass
                # Publish the content-free source/digest binding before any
                # body blob.  A crash can therefore leave a repairable
                # dangling record, never an unenumerable source-bearing blob.
                if after_digest:
                ...
                if durable is None or durable.sealed_digest != record.sealed_digest:
                    raise MemoryEditReviewIntegrityError(
                        "sealed review disappeared or conflicts after publication"
                    )
                return durable
        ...
        with self.erasure_store.document_lock(
            ...
            with self._owner_review_lock(record.tenant_id, record.owner_user_id):
                ...
                if current is None or current.sealed_digest != record.sealed_digest:
                    raise MemoryEditReviewIntegrityError(
                        "review transition is detached from its sealed proposal"
                    )
    ...
    def purge_document(self, tenant_id: str, owner_user_id: str, document_id: str) -> int:
        ...
        with self._owner_review_lock(tenant, owner):
            ...
            # A body blob is content-addressed within its target document.  If
            # another proposal shares one doomed digest, retaining that record
            # would leave a dangling reference.  Invalidate its complete body
            # set as well and continue until the reference closure is stable.
            changed = True
            while changed:
                changed = False
                for proposal_id, record in records.items():
    ...
    def _owner_records(
        ...
        for name in sorted(names):
            if not name.endswith(".json"):
                raise MemoryEditReviewIntegrityError(
                    "review purge encountered an unexpected artifact"
                )
            ...
            if record is None:
                raise MemoryEditReviewIntegrityError(
                    "review record disappeared during bounded owner enumeration"
                )
        ...
        try:
            ...
            for name in names:
                ...
                if not name.endswith(".blob") or not _is_sha256(digest):
                    raise MemoryEditReviewIntegrityError(
                        "review purge encountered an unexpected blob artifact"
                    )
                metadata = os.stat(name, dir_fd=descriptor, follow_symlinks=False)
                if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
                    raise MemoryEditReviewIntegrityError(
                        "review purge encountered a non-regular blob artifact"
                    )
                digests.add(digest)
        finally:
            os.close(descriptor)
    ...
    try:
        for entry in iterator:
            if len(names) >= maximum:
                raise MemoryEditReviewIntegrityError(
                    f"{label} enumeration exceeds its hard limit"
                )
