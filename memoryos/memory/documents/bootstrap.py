"""Crash-resumable first-user bootstrap for the controlled Markdown tree."""
...
from memoryos.memory.documents.layout import RuntimeResetRequired, tenant_control_root
from memoryos.memory.documents.model import ABSENT, PresentPath, ScanGeneration
...
class MemoryDocumentBootstrapper:
    ...
    def status(self, tenant_id: str, owner_user_id: str) -> str:
        """Return one validated durable bootstrap state without changing it."""
    ...
    def ensure_adopted_user(
        ...
        *,
        ...
        """Crash-safely initialize templates around one already-adopted file."""
    ...
    def _publish_prepared_root(self, scan: ScanGeneration) -> None:
        """Bind the safe PREPARED tree before making completion durable."""
    ...
    def _verify_completed_root(self, scan: ScanGeneration) -> None:
        """A COMPLETED marker is valid only after its immutable root binding."""
    ...
    def _probe(self, tenant: str, owner: str) -> None:
        ...
        if callable(probe):
            # The user tree may be a nested mount even when the control root is
            # local. Verify the source filesystem before any system write.
            probe(tenant, owner)
