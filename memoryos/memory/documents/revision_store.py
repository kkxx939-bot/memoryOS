"""Immutable exact-byte history for Markdown memory documents."""
...
from memoryos.core.durable_io import atomic_create_bytes, atomic_create_json
from memoryos.core.durable_io.atomic_file import _open_control_parent
from memoryos.memory.documents.control_store import DocumentCommitIntent
...
from memoryos.memory.documents.model import AbsentPath, DocumentChangeEvent, DocumentEditKind, PresentPath
...
class DocumentRevisionIntegrityError(RuntimeError):
    """A revision record or immutable content blob failed validation."""
...
class MemoryDocumentRevisionStore:
    """Protected immutable blobs plus content-free revision manifests."""
    ...
    def prune_unreferenced_blobs(
        ...
        """Remove staged plaintext that has no intent or revision authority."""
    ...
    def purge_document(self, tenant_id: str, owner_user_id: str, document_id: str) -> int:
        """Durably remove every enumerable body blob and revision row for one document."""
...
def _purge_flat_directory(
    ...
    *,
    ...
    """Unlink a known flat artifact directory without following entries."""
