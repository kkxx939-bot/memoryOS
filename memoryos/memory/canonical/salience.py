...
_PROFILE = re.compile(r"(?i)(?:^i\s+am\b|^i\s+work\b|^我是|^我在.+工作|长期从事|负责人)")
...
_DECISION = re.compile(
    ...
    r"正式决定|正式改成|确认采用|确认|决定|已采用|继续使用|保持使用)"
)
...
class EpisodeSalienceGate:
    ...
    def evaluate(
        ...
        *,
        ...
        scope_keys = tuple(sorted(scope.key for scope in episode.legal_scope_candidates()))
        budget_key = canonical_digest([episode.tenant_id, episode.episode_id, scope_keys])
