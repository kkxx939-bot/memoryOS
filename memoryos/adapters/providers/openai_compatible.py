...
from memoryos.ports.providers.embedding_provider import normalize
...
def _post_json(
    ...
    for attempt in range(max(0, retries) + 1):
        try:
            ...
            last_error = APIProviderError(f"API request failed with HTTP {exc.code}: {detail}")
            if exc.code not in {429, 500, 502, 503, 504} or attempt >= retries:
                raise last_error from exc
        except urllib.error.URLError as exc:
            last_error = APIProviderError(f"API request failed: {exc.reason}")
            if attempt >= retries:
                raise last_error from exc
        except json.JSONDecodeError as exc:
            raise APIProviderError(f"API response is not valid JSON: {exc}") from exc
        time.sleep(backoff_seconds * (2 ** attempt))
    raise APIProviderError(f"API request failed after retries: {last_error}")
...
class OpenAICompatibleEmbeddingProvider:
    ...
    def _embed_input(self, input_value: str | list[str]) -> list[list[float]]:
        response = _post_json(
            base_url=self.base_url,
            endpoint="/embeddings",
            payload={
        ...
        for item in data:
            ...
            vectors.append(normalize([float(value) for value in embedding]))
        texts = [input_value] if isinstance(input_value, str) else input_value
        if len(vectors) != len(texts):
            raise APIProviderError("Embedding response count does not match input count")
        return vectors
