"""Structured, tenant-safe SQLite Catalog predicates."""
...
class BaseFilterBuilder:
    """Compile trusted structured filters against the ``contexts`` alias ``c``."""
        ...
        *,
        ...
        for filter_name, column in _PLURAL_FILTER_ALIASES.items():
            if filter_name in normalized:
                if filter_name == "document_kinds" and normalized.get(
                    "document_kinds_apply_to_documents_only"
                ):
                    continue
                add_values(column, normalized[filter_name])
        if normalized.get("document_kinds_apply_to_documents_only"):
            ...
            clauses.append(
                "(c.record_kind NOT IN ('memory_document', 'memory_block') "
                f"OR c.document_kind IN ({placeholders}))"
            )
        ...
        if principal is not None:
            grants = ["(ag.grant_kind = 'principal' AND ag.grant_id = ?)", "ag.grant_kind = 'public'", "ag.grant_kind = 'tenant'"]
            grant_params: list[Any] = [str(principal)]
            service = normalized.get("service_access_id")
            if service:
                grants.append("(ag.grant_kind = 'service' AND ag.grant_id = ?)")
                grant_params.append(str(service))
            ...
            if workspace_values is not None:
                ...
                if workspaces:
                    ...
                    grants.append(
                        f"(ag.grant_kind = 'workspace' AND ag.grant_id IN ({placeholders}))"
                    )
                    grant_params.extend(workspaces)
            clauses.append(
                "EXISTS (SELECT 1 FROM context_acl_grants AS ag "
                "WHERE ag.tenant_id = c.tenant_id AND ag.record_key = c.record_key AND ("
                + " OR ".join(grants)
                + "))"
            )
            ...
            clauses.append(
                ...
                "AND ag.grant_kind IN ('public', 'tenant'))"
            )
