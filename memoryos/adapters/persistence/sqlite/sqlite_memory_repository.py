...
from memoryos.ports.providers.rerank_provider import RerankProvider, rerank_with_fallback
from memoryos.security.path_safety import safe_join, safe_relative_path, validate_identifier
from memoryos.services.memory.lifecycle import classify_lifecycle, hotness_score
from memoryos.services.memory.markdown import parse_memory_markdown, render_memory_markdown
from memoryos.services.memory.schema import MEMORY_TYPE_SPECS, memory_type_spec, render_template, validate_metadata
...
class MemoryStore:
    ...
    def add_memory(self, item: MemoryItem) -> Path:
        ...
        operation_id = self._operation_id("add_memory", metadata)
        self._append_operation_log(item.user_id, operation_id, "pending", "add_memory", {"path": item.path})
        try:
            ...
            raise
        self._append_operation_log(item.user_id, operation_id, "committed", "add_memory", {"path": item.path})
        return path
    ...
    def update_memory(
        ...
        operation_id = self._operation_id("update_memory", metadata)
        self._append_operation_log(user_id, operation_id, "pending", "update_memory", {"path": current["path"]})
        try:
            ...
            self._append_operation_log(
                ...
                "failed",
                "update_memory",
                {"path": current["path"], "error": str(exc)},
            )
            raise
        self._append_operation_log(user_id, operation_id, "committed", "update_memory", {"path": current["path"]})
    ...
    def delete_memory(self, identifier: str, user_id: str) -> dict[str, Any]:
        ...
        path = safe_join(self.root, current["path"])
        metadata, body = self.read_memory(current["path"])
        operation_id = self._operation_id("delete_memory", metadata)
        self._append_operation_log(user_id, operation_id, "pending", "delete_memory", {"path": current["path"]})
        tombstone = self._append_tombstone(
            user_id=user_id,
            metadata=metadata,
            body=body,
        ...
        try:
            ...
            self._append_operation_log(
                ...
                "failed",
                "delete_memory",
                {"path": current["path"], "tombstone": tombstone, "error": str(exc)},
            )
            raise
        self._append_operation_log(
            user_id,
            ...
            "committed",
            "delete_memory",
            {"path": current["path"], "tombstone": tombstone},
        )
        return {"uri": current["path"], "metadata": metadata, "deleted_content": body, "tombstone": tombstone}
    ...
    def verify_index(self, user_id: str | None = None) -> dict[str, Any]:
        ...
        if base.exists():
            for path in base.rglob("*.md"):
                ...
                if user_id and metadata.get("user_id") != user_id:
                    continue
                markdown_paths.add(str(PurePosixPath(path.relative_to(self.root).as_posix())))
                markdown_ids.add(str(metadata["id"]))
        with self._connect() as conn:
            if user_id:
                rows = conn.execute("SELECT id, path FROM memories WHERE user_id = ?", (user_id,)).fetchall()
            else:
                rows = conn.execute("SELECT id, path FROM memories").fetchall()
        index_paths = {str(row["path"]) for row in rows}
        index_ids = {str(row["id"]) for row in rows}
        return {
            "user_id": user_id,
            "markdown_count": len(markdown_paths),
            "index_count": len(index_paths),
            "missing_in_index": sorted(markdown_paths - index_paths),
            "missing_files": sorted(index_paths - markdown_paths),
            "id_mismatches": sorted(markdown_ids ^ index_ids),
            "healthy": markdown_paths == index_paths and markdown_ids == index_ids,
        }
    ...
    def _operation_id(self, operation: str, payload: dict[str, Any]) -> str:
        material = json.dumps(
            {"operation": operation, "payload": payload, "created_at": utc_now()},
            ensure_ascii=False,
            sort_keys=True,
            default=str,
        )
    ...
    def _append_operation_log(
        ...
        path = self.root / "user" / user_id / "events" / "memory_operations.jsonl"
        path.parent.mkdir(parents=True, exist_ok=True)
        record = {
    ...
    def _append_tombstone(self, user_id: str, metadata: dict[str, Any], body: str, reason: str) -> str:
        ...
        record = {
            "deleted_at": utc_now(),
            "reason": reason,
            "previous_path": metadata.get("path"),
            "metadata": metadata,
            "content": body,
        }
        with path.open("a", encoding="utf-8") as fp:
