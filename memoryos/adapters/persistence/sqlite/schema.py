"""Greenfield SQLite schema for the rebuildable Unified Context Catalog."""
...
_AUXILIARY_TABLE_UNIQUE_IDENTITIES = {
    ...
    "memory_document_projection_state": frozenset(
        {("tenant_id", "owner_user_id", "relative_path")}
    ),
}


class SchemaManager:
    """Create and validate the one supported greenfield Catalog layout."""
    ...
    def _init_db(self) -> None:
        with self._store._connect() as conn:
            ...
            existing = conn.execute(
                "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'contexts'"
            ).fetchone()
            if existing is None:
    ...
    def _validate_auxiliary_tables(conn: sqlite3.Connection) -> None:
        for table_name, expected_columns in _AUXILIARY_TABLE_COLUMNS.items():
            ...
            if (
                ...
                raise RuntimeError(
                    f"unsupported Catalog auxiliary layout for {table_name}; "
                    "reset the greenfield runtime"
                )

    def _ensure_fts_table(self, conn: sqlite3.Connection) -> bool:
        row = conn.execute(
            "SELECT sql FROM sqlite_master WHERE name = 'contexts_fts' AND type = 'table'"
        ).fetchone()
        desired = {
            "tenant_id",
            "record_key",
