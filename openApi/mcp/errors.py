"""MCP 工具调用的稳定错误协议与安全错误转换。

内部异常在这里映射为 Agent 可处理的错误码，并在返回前清理敏感路径、令牌和
不受信任的细节。
"""
...
from openApi.sdk.http_client import RemoteMemoryOSError
from sanitization import sanitize_error_text


class MCPErrorCode:
    """外部 Agent 可稳定处理的 MCP 错误码集合。"""
