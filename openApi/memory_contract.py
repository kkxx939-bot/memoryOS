"""HTTP、MCP 和 SDK 共用的 Markdown 记忆命令契约。

这里统一定义操作名、请求字段、返回结构以及边界校验，不执行记忆写入。本地用户
和内部存储命名空间不属于命令载荷，由进程配置绑定，避免普通输入直接决定持久化身份。
"""
...
MEMORY_COMMAND_REQUEST_SCHEMAS["merge_propose"] = deepcopy(MEMORY_COMMAND_REQUEST_SCHEMAS["merge"])
...
MEMORY_COMMAND_RESPONSE_SCHEMAS: dict[str, dict[str, Any]] = {
    ...
    "merge": {
        ...
        "properties": {
            ...
            "preserved_source_document_ids": {"type": "array", "items": {"type": "string"}},
        },
        "required": [
            "saga_id",
            "status",
...
MEMORY_COMMAND_RESPONSE_SCHEMAS["merge_resume"] = deepcopy(MEMORY_COMMAND_RESPONSE_SCHEMAS["merge"])
MEMORY_COMMAND_RESPONSE_SCHEMAS["merge_propose"] = deepcopy(MEMORY_COMMAND_RESPONSE_SCHEMAS["review_preview"])
