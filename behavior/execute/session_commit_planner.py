"""根据 Session 证据规划 Behavior 案例、聚类、模式和支撑锚点。"""
...
from behavior.projection.behavior_pattern import BehaviorPatternUpdater
from behavior.projection.behavior_support import BehaviorSupportWriter
...
class BehaviorCommitPlanner:
    def __init__(self, index_store: IndexStore | None = None, source_store: SourceStore | None = None) -> None:
        ...
        self.support_writer = BehaviorSupportWriter()
    ...
    def _anchor(self, user_id: str, scene_key: str, case_refs: list[str]) -> BehaviorSupportAnchor:
        anchor_key = f"{scene_key}_anchor"
        return BehaviorSupportAnchor(
    ...
    def _support_anchor_operation(
        ...
        *,
        ...
        if self.source_store is None:
            return self.support_writer.add(anchor, evidence=evidence)
        try:
        ...
        return self.support_writer.update(
            anchor,
