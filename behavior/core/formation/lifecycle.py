...
class BehaviorLifecycleResult:
    ...
    support_anchor: BehaviorSupportAnchor | None = None
    support_candidate_required: bool = False


class BehaviorLifecycleService:
    """根据行为时间窗口决定继续保留案例、形成行为簇或升级为行为模式。"""
    ...
    def _anchor(
        ...
        return BehaviorSupportAnchor(
            ...
            supporting_behavior_uris=[
                f"memoryos://user/{user_id}/behavior/cases/{scene_key}/{case.case_id}"
                for case in cases
            ],
        )
