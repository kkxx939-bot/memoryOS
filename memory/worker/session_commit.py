...
from infrastructure.store.memory.control_store import DocumentControlIntegrityError
from infrastructure.store.session.commit_group import CommitGroupIntegrityError
from memory.commit.document_commit import DocumentCommitConflict
from memory.commit.evidence.errors import EvidenceArchiveError
...
class SessionCommitWorker:
    ...
    def process_pending(
        ...
        *,
        ...
        # 提交组恢复和队列租赁之前都必须检查就绪状态；未就绪运行时既不能
        # 提取记忆，也不能修改耐久任务。
        require_session_service_ready(self.service)
