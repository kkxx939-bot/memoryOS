...
export class MemoryOSContextEngine {
  ...
  constructor(private readonly client: MemoryOSHttpClient, private readonly checkpointEvery = 10) {}
  async assemble(ctx: SessionContext): Promise<MemoryOSResponse> {
    return this.client.request("/v1/context/assemble", {query:ctx.prompt || ctx.projectId, user_id:ctx.userId, project_id:ctx.projectId, connect_metadata:{adapter_id:"openclaw"}});
  }
