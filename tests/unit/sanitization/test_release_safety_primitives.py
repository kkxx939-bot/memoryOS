...
def test_recall_trace_redacts_secret_query_and_uses_private_file(tmp_path: Path) -> None:
    repository = RecallTraceRepository(tmp_path / "traces")
    service = RecallTraceService(repository)
    trace_id = service.record(
        "OPENAI_API_KEY=sk-live-secret",
        scope={"user_id": "u1", "tenant_id": "default"},
    )
    trace = service.read(trace_id)
    assert "query" not in trace
    assert trace["query_digest"] == hashlib.sha256(b"OPENAI_API_KEY=sk-live-secret").hexdigest()
    assert trace["query_utf8_bytes"] == len(b"OPENAI_API_KEY=sk-live-secret")
    path = repository.trace_root / f"{trace_id}.json"
...
def test_recall_trace_sanitizes_all_fields_and_fails_closed(tmp_path: Path) -> None:
    ...
    trace_id = service.record(
        "report",
        scope={"user_id": "u1", "tenant_id": "default"},
        metadata_filters={"cookie": "session=never-write-this-cookie"},
        selected=[
            {
    ...
    encoded = json.dumps(service.read(trace_id), ensure_ascii=False)
    ...
    before = set(repository.trace_root.glob("*.json"))
    with pytest.raises(ValueError, match="sanitization unavailable"):
        service.record("safe query", scope={"user_id": "u1", "tenant_id": "default"})
    assert set(repository.trace_root.glob("*.json")) == before
