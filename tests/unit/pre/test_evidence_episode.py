...
def test_default_observation_actor_is_sensor_and_tool_output_is_text() -> None:
    archive = SessionArchive(
        user_id="u1",
        session_id="source-kinds",
        archive_uri="memoryos://user/u1/sessions/history/source-kinds",
        observations=[{"id": "o1", "raw_text": "Remember this sensor output"}],
        tool_results=[{"id": "t1", "tool_output": "bounded tool result"}],
        created_at="2026-01-01T00:00:00Z",
    )

    episode = SessionArchiveEpisodeAdapter().adapt(archive)
    observation = episode.event("o1")
    tool_result = episode.event("t1")

    assert observation is not None and observation.actor.kind == "sensor"
    assert tool_result is not None and tool_result.text() == "bounded tool result"
