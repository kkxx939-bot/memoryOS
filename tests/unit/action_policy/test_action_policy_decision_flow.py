...
from behavior.projection import behavior_support_to_context_object
...
from policy.action_policy.decision import ActionContext, PolicyGate, PredictionEngine, PredictionRequest
from policy.action_policy.model import ActionPolicy, ActionPolicyStatus, PenaltySignal, RewardSignal
from policy.action_policy.ranking import ActionPolicyRanker
from policy.action_policy.update import ActionPolicyUpdater, FeedbackCommitPlanner
...
from transaction.model import OperationAction
...
class PredictiveContextDatabaseTest(unittest.TestCase):
    ...
    def test_behavior_lifecycle_creates_anchor_cluster_and_pattern(self) -> None:
        ...
        self.assertIsInstance(result.support_anchor, BehaviorSupportAnchor)
        ...
        self.assertTrue(
            all(uri.startswith("memoryos://user/gulf/behavior/cases/hot/") for uri in result.support_anchor.supporting_behavior_uris)
        )
    ...
    def test_policy_gate_allows_auto_execute_only_after_gate(self) -> None:
        ...
        decision = PolicyGate().evaluate(candidate, action_context, action_policy=policy, decision_confidence=0.8)
    ...
    def test_prediction_engine_records_ledger_and_does_not_write_memory(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = PredictionEngine(index, ActionPolicyDecisionLedger(tmp)).process(
                PredictionRequest(
                    user_id="gulf",
                    episode_id="ep1",
    ...
    def test_prediction_engine_propagates_verified_exact_anchor_to_rank_and_gate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source.write_object(behavior_support_to_context_object(anchor), content=anchor.content)
            policy = ActionPolicy(
                user_id="gulf",
                scene_key="hot",
                action="turn_on_fan",
            ...
            result = PredictionEngine(
                index,
                ActionPolicyDecisionLedger(tmp),
                source_store=source,
            ).process(
                PredictionRequest(
                    user_id="gulf",
    ...
    def test_prediction_engine_rejects_caller_supplied_cross_user_policy(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = PredictionEngine(InMemoryIndexStore(), ActionPolicyDecisionLedger(tmp)).process(
                PredictionRequest(
                    user_id="gulf",
