...
from memory.commit.model.session import SessionCommitResult
from openApi.sdk import ProcessObservationResult
...
def _client(
    ...
    client.runtime = SimpleNamespace(
        policy=SimpleNamespace(
            engine=FakeEngine(prediction_result or _prediction_result()),
            executor=FakeExecutor(action_result or _action_result()),
        ),
        session=SimpleNamespace(
            commit_service=FakeContextDB(commit_result),
            archive_store=None,
        ),
        ...
        readiness=SimpleNamespace(require_ready=lambda: None),
    )
...
def test_process_observation_without_archive_executes_action_and_does_not_commit() -> None:
    ...
    assert isinstance(result, ProcessObservationResult)
    assert len(client.runtime.policy.engine.calls) == 1
    assert len(client.runtime.policy.executor.calls) == 1
    assert client.runtime.session.commit_service.calls == []
    assert result.prediction_result is client.runtime.policy.engine.result
    assert result.action_result is client.runtime.policy.executor.result
...
def test_process_observation_archive_commit_result_and_async_flag_are_visible() -> None:
    ...
    assert len(client.runtime.session.commit_service.calls) == 1
    archive, async_commit = client.runtime.session.commit_service.calls[0]
...
def test_process_observation_binds_archive_to_client_tenant() -> None:
    ...
    archive, _async_commit = client.runtime.session.commit_service.calls[0]
...
def test_process_observation_archive_passes_async_true() -> None:
    ...
    assert client.runtime.session.commit_service.calls[0][1] is True
...
def test_predict_returns_prediction_result_without_action_or_commit_when_action_capable() -> None:
    ...
    assert len(client.runtime.policy.engine.calls) == 1
    assert client.runtime.policy.executor.calls == []
    assert client.runtime.session.commit_service.calls == []


def test_commit_agent_session_returns_session_service_result() -> None:
    ...
    assert client.runtime.session.commit_service.calls[0][1] is False
