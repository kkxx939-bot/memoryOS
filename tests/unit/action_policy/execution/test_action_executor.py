...
def test_execute_blocks_explicitly_non_executable_skill() -> None:
    context = _context()
    context.packed_context["slices"]["skill"]["items"][0]["metadata"]["executable"] = False

    result = ActionExecutor().execute(
        PolicyDecision(mode="execute", allowed=True, action="turn_on_ac", reason="ok"),
        context,
    )

    assert result.status == "blocked"
    assert result.reason == "Required skill is not executable."
