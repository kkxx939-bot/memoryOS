...
class PolicyGateContextRulesTest(unittest.TestCase):
    ...
    def test_action_spec_confirmation_cannot_be_bypassed_by_stored_policy(self) -> None:
        policy = ActionPolicy(
            user_id="u1",
            scene_key="hot",
            action="turn_on_ac",
        ...
        candidate = ActionCandidate(action=policy.action, score=0.95, policy_uri=policy.uri, reason="test")

        decision = PolicyGate().evaluate(candidate, self.context(), policy, 0.95)

        self.assertFalse(policy.auto_execute_allowed)
        self.assertEqual(decision.mode, "ask_user")
