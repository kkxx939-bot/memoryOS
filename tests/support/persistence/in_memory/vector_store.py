...
from infrastructure.store.contracts.vector import VectorCapabilities, VectorHit, vector_row_id


class InMemoryVectorStore:
    """在进程内维护向量和元数据索引，用于隔离依赖向量存储的测试。"""
    ...
    def search_vector(self, embedding: list[float], namespace: str, limit: int = 10) -> list[VectorHit]:
        ...
        for uri, (stored, metadata) in self.rows.items():
            if namespace:
                ...
                if not (uri.startswith(namespace) or metadata_namespace == namespace or tenant_id == namespace):
                    continue
