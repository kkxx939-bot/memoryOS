...
def _seed_client(tmp_path, handler):
    ...
    client.context_db.write_object(ContextObject(uri=anchor_uri, context_type=ContextType.MEMORY, title="hot anchor", owner_user_id="u1"), content="hot anchor")
    client.context_db.write_object(ContextObject(uri=resource_uri, context_type=ContextType.RESOURCE, title="AC", metadata={"available": True}), content="available")
    client.context_db.write_object(ContextObject(uri=skill_uri, context_type=ContextType.SKILL, title="AC skill", metadata={"tool_name": "ac_tool", "executable": True}), content="tool")
    policy = ActionPolicy(
        user_id="u1",
...
def test_execute_failure_writes_failed_action_result_and_penalizes_policy(tmp_path) -> None:
    ...
    client.process_observation(request, [policy], archive_session=True, async_commit=True)
