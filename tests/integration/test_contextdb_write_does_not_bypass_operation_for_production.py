...
def test_contextdb_production_write_uses_operation_committer_and_seed_is_explicit(tmp_path) -> None:
    ...
    obj = ContextObject(uri="memoryos://user/u1/memories/profile/prod", context_type=ContextType.MEMORY, title="prod", owner_user_id="u1")
    operation = ContextOperation(user_id="u1", context_type=ContextType.MEMORY, action=OperationAction.ADD, target_uri=obj.uri, payload={"context_object": obj.to_dict(), "content": "prod content"})
    ...
    seed = ContextObject(uri="memoryos://user/u1/memories/profile/seed", context_type=ContextType.MEMORY, title="seed", owner_user_id="u1")
