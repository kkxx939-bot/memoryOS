...
from sanitization.text import sanitize_text
...
class ContextProjectionSanitizer:
    ...
    def sanitize(
        ...
        *,
        ...
        try:
            ...
            raise
        except Exception as exc:  # 派生索引不能接收只完成部分清洗的数据。
    ...
    def sanitize_tree_segments(self, values: Sequence[object]) -> tuple[str, ...]:
        """为逻辑树路径返回稳定且不包含敏感信息的动态片段。

        树路径是 Serving 分类而不是证据。Schema 管理的合法 ID 保持可读；凭证或
        类似用户目录链的值使用确定性摘要，不能原样进入 Catalog 或
        ``context_paths``。一旦某个片段属于用户目录链，同级动态片段也全部摘要化，
        避免用户名残留在已清洗的 ``Users``/``home`` 标记旁边。
        """
