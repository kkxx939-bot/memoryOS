...
class PolicyGate:
    def evaluate(
        ...
        if candidate.policy_uri != action_policy.uri or canonical_action(candidate.action) != action_policy.action:
            return PolicyDecision(
                mode="blocked",
                allowed=False,
                action="do_nothing",
                reason="Candidate does not match its ActionPolicy.",
            )
        ...
        if action_policy.status in {
            ActionPolicyStatus.SUPPRESSED,
            ActionPolicyStatus.OBSOLETE,
            ActionPolicyStatus.DELETED,
        }:
            return PolicyDecision(mode="suppress", allowed=False, action="do_nothing", reason="ActionPolicy is unavailable.")
        ...
        if action_policy.status == ActionPolicyStatus.COOLDOWN and (
            not action_policy.cooldown_until or self._is_future(action_policy.cooldown_until)
        ):
            return PolicyDecision(mode="ask_user", allowed=True, action="ask_user", reason="ActionPolicy cooldown is still active.")
        if action_policy.status != ActionPolicyStatus.COOLDOWN and action_policy.cooldown_until and self._is_future(action_policy.cooldown_until):
            return PolicyDecision(mode="ask_user", allowed=True, action="ask_user", reason="ActionPolicy cooldown is still active.")
        if spec.executable:
            if spec.requires_confirmation:
                return PolicyDecision(mode="ask_user", allowed=True, action="ask_user", reason="Action requires confirmation.")
            if (
                action_policy.auto_execute_allowed
                and spec.risk_level in {"none", "low"}
