...
- `memory/editor` 已重新建立为独立领域包，但旧数据契约仍然退出，不保留 `MemoryEditSource`、`MemoryEditBatch`、公开导出或兼容入口。Memory Editor 的输入输出、可信来源绑定、LLM 结构化候选、旧记忆读取、合并和写入编排均等待逐项重新确认；当前包不得提前实现或用占位逻辑推断。
