...
class SQLiteRelationStore:
    ...
    def relations_of(
        ...
        *,
        ...
        resolved_tenant = self._require_tenant(tenant_id)
        sql = "SELECT * FROM relations WHERE tenant_id = ? AND (source_uri = ? OR target_uri = ?)"
    ...
    def all_relations(self, *, tenant_id: str) -> list[ContextRelation]:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM relations WHERE tenant_id = ? ORDER BY source_uri, relation_type, target_uri",
                (resolved_tenant,),
            ).fetchall()
        return [self._relation_from_row(row) for row in rows]
    ...
    def _init_db(self) -> None:
        with self._connect() as conn:
            row = conn.execute("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'relations'").fetchone()
            if row is None:
