"""基于 SQLite、具备租户隔离的统一 Context Catalog。"""
...
from infrastructure.store.sqlite.base_filter import BaseFilterBuilder
from infrastructure.store.sqlite.catalog import CatalogStoreOperations
from infrastructure.store.sqlite.connection import SQLiteConnectionManager
from infrastructure.store.sqlite.query_filters import QueryFilterBuilder
from infrastructure.store.sqlite.schema import SchemaManager
from infrastructure.store.sqlite.search import CatalogSearchOperations
from infrastructure.store.sqlite.serialization import CatalogSerializer
from infrastructure.store.sqlite.tombstones import TombstoneOperations


class SQLiteIndexStore:
    """所有接口都显式携带租户归属的 Catalog 门面。"""
    ...
    def serving_generation_token(self) -> str:
        ...
        for path in (
            ...
            try:
                ...
                continue
            digest.update(
                f"{path.name}:{details.st_dev}:{details.st_ino}:{details.st_size}:{details.st_mtime_ns}".encode()
            )
