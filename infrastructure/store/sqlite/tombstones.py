...
class TombstoneOperations:
        ...
        *,
        ...
        with self._store._connect() as conn:
            for offset in range(0, len(ordered_ids), _MAX_FILTER_VALUES):
                ...
                rows = conn.execute(
                    f"SELECT * FROM context_tombstones WHERE tenant_id = ? AND tombstone_id IN ({placeholders})",
    ...
    def _mark_failed(
        ...
        *,
        ...
        with self._store._connect() as conn:
            status_sql = " AND status = 'CLEANING'" if require_cleaning else " AND status NOT IN ('APPLIED', 'STALE')"
            ...
            conn.execute(
                ...
                "last_error = ?, updated_at = ? WHERE tenant_id = ? AND tombstone_id = ?" + status_sql,
