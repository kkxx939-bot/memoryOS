...
class CatalogRecord:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "workspace_shared",
            _workspace_is_shared(metadata, workspace_id=workspace_id),
        )
    ...
    def with_sanitized_projection(self, sanitizer: ContextProjectionSanitizer | None = None) -> CatalogRecord:
        ...
        # Catalog 列是逻辑位置的权威来源；路径经过受控分类规范化后，元数据不得保留原始路径。
    ...
    def from_context_object(
        ...
        *,
        ...
        # 普通上下文投影以 URI 为键；领域投影必须显式传入稳定且限定租户的记录键。
...
def catalog_vector_metadata(
    ...
    *,
    ...
    向量数据库属于可重建 Serving 层，每条记录必须携带精确 Catalog 身份和 Top-K 前
    过滤所需的可信结构化维度；这里只发布类型化字段和已校验范围键。
    """
...
def _workspace_is_shared(
    metadata: Mapping[str, Any],
    *,
    workspace_id: str,
) -> bool:
    """读取显式业务共享标记；它只影响同一本地用户的工作区检索。"""

    return bool(workspace_id and metadata.get("workspace_shared") is True)
...
def _validate_taxonomy_shape(segments: Sequence[str]) -> None:
    """保证所有逻辑路径都位于有限且由 Schema 控制的分类体系内。

    动态标识只能位于结构固定的根节点下，避免模型或调用方构造无界目录树，同时允许
    根路径和 Timeline 部分前缀查询。
    """
