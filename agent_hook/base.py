"""Agent Hook 公共执行流程：召回上下文、记录事件、提交会话与失败重试。"""
...
from agent_hook.injection import MemoryOSContextRenderer
from agent_hook.queue import PendingItem, PendingQueue
...
class BaseAgentHookAdapter:
    def __init__(
        ...
        *,
        ...
        if queue is not None and queue.user_id != config.user_id:
            raise ValueError("pending hook queue user does not match AgentHookConfig")
    ...
    def assemble_context(self, event: AgentHookEvent) -> HookResult:
    ...
    def _append_event(self, event: AgentHookEvent):
        # 旧持久化格式保留固定命名空间字段，但外部不可选择或配置。
        event.tenant_id = LOCAL_STORAGE_NAMESPACE
